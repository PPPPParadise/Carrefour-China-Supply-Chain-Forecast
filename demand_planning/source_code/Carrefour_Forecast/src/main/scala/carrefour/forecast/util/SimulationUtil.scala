...
object SimulationUtil {
  ...
  /**
    * Insert store stock level generated by simulation process to datalake
    * 将模拟运行计算的门店库存水平写入datalake
    *
    * @param resDf Store stock level generated by simulation process 模拟运行计算的门店库存水平
    * @param modelRun Job run information 脚本运行信息
    * @param sqlc Spark SQLContext
    */
  ...
  /**
    * Insert item information used by simulation process to datalake
    * 将模拟运行使用的商品信息写入datalake
    *
    * @param resDf Item information used by simulation process 模拟运行使用的商品信息
    * @param modelRun Job run information 脚本运行信息
    * @param sqlc Spark SQLContext
    */
  def insertSimulationItemInfoToDatalake(resDf: DataFrame, modelRun: ModelRun, sqlc: SQLContext): Unit = {
    val simulationItemInfoView = modelRun.flowType + "_simulation_item_status_df"

    resDf.createOrReplaceTempView(simulationItemInfoView)
