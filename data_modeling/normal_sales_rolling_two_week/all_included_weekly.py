...
def main():
    ...
    spark_df = spark_df.withColumn("insert_date",int(datetime.datetime.now().strftime("%Y%m%d")))
    spark_df.write.format("parquet").mode('overwrite').partitionBy("insert_date").saveAsTable(f"{config['database']}.result_forecast_10w_on_the_fututre")
    spark.stop()
