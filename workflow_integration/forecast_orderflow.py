...
weekly_forecast_filename = Variable.get("weekly_forecast_filename").strip()
...
def python_store_order_file(ds, **kwargs):
    order_day = get_order_day(kwargs['tomorrow_ds_nodash'])
    store_order_file_process(order_day, record_folder, order_output_folder, datachecking_output_folder,
                             store_order_filename.format(order_day), store_highvalue_order_filename.format(order_day),
                             xdock_high_volume_order_filename.format(order_day))
...
def python_store_missing_order_file(ds, **kwargs):
    order_day = get_order_day(kwargs['tomorrow_ds_nodash'])
    store_missing_order_process(order_day, record_folder, datachecking_output_folder,
                                store_missing_order_filename.format(order_day))


def python_dc_missing_order_file(ds, **kwargs):
    order_day = get_order_day(kwargs['tomorrow_ds_nodash'])
    dc_missing_order_process(order_day, record_folder, datachecking_output_folder,
                             dc_missing_order_filename.format(order_day))
...
generate_forecast_file = PythonOperator(task_id='generate_forecast_file',
                                python_callable=python_generate_forecast_file,
                                provide_context=True,
                                dag=forecast_orderflow)
