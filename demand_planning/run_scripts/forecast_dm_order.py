...
def insert_script_run(date_str, status, output_str, info_str, error_str, sqlc):
    sql = \
        """
    ...
    sql = sql.format(date_str, status, output_str, info_str, error_str)
    sqlc.sql(sql)


def run_dm_order(ds, **kwargs):
    warehouse_location = abspath('spark-warehouse')
    os.environ["PYSPARK_SUBMIT_ARGS"] = '--jars /data/jupyter/kudu-spark2_2.11-1.8.0.jar pyspark-shell'

    print_output('Job start')
    ...
    print_output("Load DM items and stores")

    dm_item_store_sql = \
        """
        SELECT ndt.dm_theme_id,
        ...
        FROM vartefact.nsa_dm_extraction_log del
    ...
    print_output(f"Before filtering already calculated DM {dm_item_store_df.count()}")
    ...
    if dm_item_store_cnt == 0:
        ...
        insert_script_run(date_str, "Success", output_str, info_str, "", sqlc)
        return
    ...
    onstock_order_sql = \
        ...
        JOIN vartefact.ordinary_onstock_order_deliver_mapping mp ON mp.dept = dis.dept
        ...
        JOIN vartefact.forecast_calendar dev ON dev.weekday_short = mp.deliver_weekday
            AND dev.week_index = ord.week_index + mp.week_shift
        WHERE to_date(ord.date_key, 'yyyyMMdd') >= to_date(dis.ppp_start_date, 'yyyy-MM-dd')
            AND dev.date_key <= '{0}'
        """.replace("\n", " ")
    ...
    xdock_order_sql = \
        ...
        JOIN vartefact.forecast_item_details id ON id.item_code = dis.item_code
            AND id.sub_code = dis.sub_code
            AND id.dept_code = dis.dept_code
        JOIN vartefact.ordinary_xrotation_order_deliver_mapping xo ON dis.item_code = xo.item_code
        ...
        JOIN vartefact.forecast_calendar dev ON dev.iso_weekday = xo.deliver_weekday
    ...
    dm_sales_predict_sql = \
        ...
        FROM vartefact.dm_pred_results_simple pred
    ...
    actual_stock_sql = \
        ...
        JOIN vartefact.item_code_id_stock icis ON icis.item_id = dp.item_id
    ...
    after_fourweek_sql = \
        ...
        FROM dm_prediction dp
        JOIN vartefact.v_forecast_daily_sales_prediction fcst ON fcst.item_id = dp.item_id
            AND fcst.sub_id = dp.sub_id
            AND fcst.store_code = dp.store_code
