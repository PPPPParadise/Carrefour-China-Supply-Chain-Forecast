...
/**
  * Utility used for simulation process<br />
  * 用于模拟运行的工具函数
  */
object SimulationUtil {

  /**
    * Insert simulation result to datalake<br />
    * 将模拟运行结果写入数据池
    *
    * @param resDf 模拟运行结果
  ...
  /**
    * Insert orders generated by simulation process to datalake<br />
    * 将模拟运行计算的订单规划写入数据池
    *
  ...
  /**
    * Get sales predictions used in simulation<br />
    * 获取模拟运行使用的销量预测
    *
    ...
    */
  def getSimulationSalesPrediction(dateMapDf: DataFrame, startDateStr: String, endDateStr: String, viewName: String,
                         sqlc: SQLContext): DataFrame = {

    // Get sales prediction
    val salesPredictionSql = SimulationQueries.getSimulationSalesPredictionSql(startDateStr, endDateStr, viewName)
  ...
  /**
    * Get on the way order quantity and delivery date from simulation process<br />
  ...
  /**
    * Get future store orders to DC generated by simulation process<br />
    * 获取模拟运行计算的门店向大仓未来订货量
    *
    * @param dateMapDf All combinations of item, store, and dates 全部商品，门店，及日期的组合
    * @param startDateStr Start date in yyyyMMdd String format 文本格式的起始日期，为yyyyMMdd格式
  ...
  /**
    * Get store actual orders to DC generated by simulation process<br />
    * 获取模拟运行计算的门店向大仓真实订货量
    *
    * @param dateMapDf All combinations of item, store, and dates 全部商品，门店，及日期的组合
    * @param startDateStr Start date in yyyyMMdd String format 文本格式的起始日期，为yyyyMMdd格式
  ...
  /**
    * Get past generated orders for DC generated by simulation process<br />
    * 获取模拟运行计算的过去生成的大仓订单规划
    *
    * @param startDateStr Start date in yyyyMMdd String format 文本格式的起始日期，为yyyyMMdd格式
    * @param endDateStr Start date in yyyyMMdd String format 文本格式的起始日期，为yyyyMMdd格式
    * @param isDcFlow Whether it is DC flow 是否为计算DC/货仓订单
    ...
    */
  def getSimulationPastOrdersMap(startDateStr: String, endDateStr: String, isDcFlow: Boolean,
                                 flowType: FlowType.Value, viewName: String,
                                 spark: SparkSession): Map[ItemEntity, List[Tuple2[String, Double]]] = {
    import spark.implicits._

    val pastOrdersSql = SimulationQueries.getSimulationPastOrdersSql(startDateStr, endDateStr, flowType, viewName)
